...
try {
    ...
    if ($grade === 'auto') {
        ...
        $rubricStmt = $pdo->prepare("
            SELECT major_exam, exercises, assignments_activities_attendance 
            FROM laboratory_rubrics 
        ...
        $exerciseWeight = $rubric['exercises'] / 100; // Changed from quizWeight
        ...
        // Calculate Exercise Score (changed from Quiz Score)
        ...
        $exerciseStmt->execute($params);
        $exerciseData = $exerciseStmt->fetch(PDO::FETCH_ASSOC);
        $exerciseScore = ($exerciseData['total_max'] > 0) ? ($exerciseData['total_score'] / $exerciseData['total_max']) * $exerciseWeight : 0;
        ...
        $totalGrade = $exerciseScore + $tripleAScore + $examScore;
