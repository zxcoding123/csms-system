				...
				<div class="container-fluid p-0">
                    ...
                    ");
							...
							<div class="container-fluid my-4">
								<h2 class="mb-4 text-center"><b>Announcements</b></h2>

								<div class="row">
									<!-- Class Announcements Column -->
									<div class="col-lg-6 mb-3">
										<h4 class="mb-3"><b>Class</b> Announcements</h4>
										<div id="class-announcements" class="row">
											<!-- Class posts will be inserted here -->
										</div>
									</div>

									<!-- Subject Announcements Column -->
									<div class="col-lg-6 mb-3">
										<h4 class="mb-3"><b>Subject</b> Announcements</h4>
										<div id="subject-announcements" class="row">
											<!-- Subject posts will be inserted here -->
										</div>
									</div>
								</div>
							</div>
							...
							$sql = "
    SELECT a.*, t.fullName AS teacher_name, t.avatar AS teacher_avatar
    FROM announcements a
    JOIN staff_accounts t ON a.teacher_id = t.id
 WHERE LOWER(a.class_name) LIKE LOWER(:class_name)
    ORDER BY a.created_at DESC
";

							$stmt = $pdo->prepare($sql);
							$stmt->execute([
								'class_name' => $classPattern
							]);
...
";
							...
							$subjects_announcements = $stmt->fetchAll(PDO::FETCH_ASSOC);


							?>
							<script>
								const classAnnouncements = <?php echo json_encode($announcements); ?>;
								const subjectAnnouncements = <?= json_encode($subjects_announcements) ?>;

								// Helper: get visible text length of rich HTML
								...
								// Helper: safely truncate rich HTML without breaking tags
								...
								function renderAnnouncements(containerId, announcements, limit = 1, showViewMore = true, viewMoreUrl = '#', charLimit = 200) {
									...
									limited.forEach((post, index) => {
										const postId = `${containerId}-post-${post.id || index}`;

										// Use visible text length to check truncation
										const contentLength = getTextLength(post.content);
										const truncated = contentLength > charLimit;
										const previewContent = truncated ? truncateHTML(post.content, charLimit) : post.content;
            ...
            <div class="card shadow-sm announcement-post">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        <img 
                            src="../uploads/profile_pictures/${post.teacher_avatar ?? 'https://i.pravatar.cc/50?u=' + post.teacher_id}"
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="small text-muted">
                                ${post.subject_name ?? post.class_name}
					...
					<hr>
                    <!-- Content Preview -->
                    <div id="content-${postId}" class="announcement-content mb-2">
                        ${previewContent}
                    </div>

                    ${truncated ? `<button class="btn btn-sm btn-link p-0" id="toggle-${postId}">Read More</button>` : ''}
                    ...
                    <div class="d-flex justify-content-between align-items-center border-top pt-2 mt-2">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button class="btn btn-sm btn-outline-primary"><i class="bi bi-share"></i> Share</button>
                        </div>
                    </div>
        ...
        `;
										...
										if (truncated) {
											...
											toggleBtn.addEventListener('click', () => {
												if (!expanded) {
													contentDiv.innerHTML = post.content; // full rich text
													toggleBtn.textContent = 'Show Less';
													expanded = true;
												} else {
													contentDiv.innerHTML = previewContent; // truncated rich text
									...
									// View More button
									if (showViewMore && announcements.length > limit) {
										container.insertAdjacentHTML('beforeend', `
            <div class="text-center mt-2">
                <a href="${viewMoreUrl}" class="btn btn-sm btn-outline-primary">View more</a>
            </div>
        `);
									}
								}
								...
								// Usage example
								renderAnnouncements('class-announcements', classAnnouncements, 1, true, 'class_announcements.php', 50);
								renderAnnouncements('subject-announcements', subjectAnnouncements, 1, true, 'subject_announcements.php', 100);
							</script>
